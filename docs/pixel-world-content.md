...
## 버그 수정 — 농작물이 캐릭터/강아지보다 항상 위에 그려짐 (2026-09-23)

"농장에 농작물이 자꾸 캐릭터랑 강아지보다도 더 위에 있어서 이상하게 보인다"는 버그 제보.

- **원인**: `.pr-farm-bed`(밭 버튼)의 z-index가 CSS에 `15`로 고정돼 있었다. 반면 플레이어
  (`.pr-plaza-actor`)는 `actor.y+2`, 강아지(`.pr-dog`)는 `Math.floor(y)+1`로 자기 행(y) 기준
  깊이 정렬을 쓴다. 밭은 y=4/y=7에 있어 두 캐릭터의 z-index가 웬만해선 15를 못 넘다 보니, 플레이어가
  밭보다 앞(아래)에 서 있어도 항상 밭이 위에 그려졌다 — 지난 라운드 허수아비 말풍선 버그와 같은
  "정적인 고정값 vs 행 기반 동적 값" 부류의 문제다.
- **수정**: 밭의 z-index를 CSS 고정값에서 `bed.y+1`(행 기반, `Farm.tsx`에서 인라인으로 계산)로
  바꿨다. 강아지(`Math.floor(y)+1`)와는 밭 자신의 영역(y/y+1행)이 애초에 걸을 수 없는 칸이라
  겹치는 행에서 절대 동점이 나지 않는다. 플레이어(`y+2`)와는 밭 바로 위 한 칸(코너 위치, 실제로
  돌보러 가는 접근 칸은 아님)에서만 이론상 동점이 나는데, 이 경우 DOM 순서상 플레이어가 이겨서
  플레이어가 밭 위에 그려진다 — 반대(강아지가 접근 칸에서 밭에 가려지는 것, 즉 이번에 신고된
  버그 자체를 강아지 기준으로 재현하는 것)보다 훨씬 덜 눈에 띄는 코너 케이스라 이쪽을 선택했다.
  플레이어/강아지가 실제로 밭에 다가가서 돌보는 자리(접근 칸)에서는 두 경우 모두 여유 있게
  올바른 순서로 그려진다.
- 검증: `tests/pixel-room/farm.browser.mjs`에 두 가지 회귀 테스트 추가 — (1) 밭에 다가가 돌보는
  순간(`bed()` 헬퍼가 리턴하는 시점, 항상 접근 칸에 도착해 있음) 플레이어의 z-index가 밭보다
  큰지 확인, (2) 강아지가 자율적으로 돌아다니는 동안 40프레임을 샘플링해 매 프레임 "강아지가 밭보다
  앞줄(y가 큼)이면 z-index도 더 커야 하고, 뒷줄이면 더 작아야 한다"는 불변식을 검사(양쪽 밭 모두,
  3개 뷰포트 × 40프레임 = 위반 0건). 개발 중 별도 스크립트로 실제 화면을 여러 지점에서 스크린샷
  찍어 육안으로도 확인했다. 전체 유닛 테스트 123/123, 기존 회귀(`base-appearance`/`doormat`/
  `furniture-race`/`yard`(390/1440)/`dog`/`pixelShop/browser`(3뷰포트)/`plaza/interactions`/
  `plaza/landscape`(390/1440)) 전부 재실행해 회귀 없음 확인. `npm run build` clean, `npm run
  lint`에는 이번 변경 파일 관련 오류/경고 없음.
