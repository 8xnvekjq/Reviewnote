...
## 17. 12차 개선 — 대기 화면을 실제 평균시간 기준 선형 카운트다운으로 교체

배경: `diagnosis_stats` 연동 후 평균 문구는 정상적으로 떴지만, "예상 대기 시간이 평균 기준으로 실제 1초씩 줄어야 하는데 지멋대로 움직인다"는 피드백을 받았습니다. 원인은 진행률(`progress`)이 실제 경과 시간과 정비례하지 않는 **연출용 곡선**(초반 40% 시간 동안 70%까지 빠르게 상승 → 90%까지 서서히 상승 → 지수함수로 99.5%에 점근)이었기 때문입니다. 이 곡선은 애초에 "그럴듯하게 보이는 연출"이 목적이었지, 실제 남은 시간을 정확히 계산하는 게 목적이 아니었습니다.

### 수정 ([MistakeDetailModal.tsx](src/components/MistakeDetailModal.tsx))
- 곡선 공식을 전부 제거하고 **`progress = min(99, 경과시간 / 평균시간 * 100)`** 형태의 순수 선형 계산으로 교체했습니다. 실제 경과 시간에 정확히 비례해서 움직입니다.
- 경과 시간을 추적하는 `elapsedMs` state를 추가하고, 이를 기반으로 남은 시간(`remainingSeconds`)과 초과 시간(`overtimeSeconds`)을 매 렌더마다 계산합니다.
- **평균 시간 초과 시 "+1초", "+2초" 형태로 표시**하도록 추가했습니다 (`amber` 색상으로 강조). 진행률 링 자체는 100%를 완료 신호로 예약해둬야 해서 99%에서 멈추고, 대신 이 초과 시간 문구가 계속 올라갑니다.
- 기존에 있던 "마지막 맞춤 처방을 다듬는 중입니다..." 같은 곡선 전용 중간 멘트는 선형 모델과 맞지 않아 제거하고, 완료/초과/남은시간 세 가지 상태로만 단순화했습니다.
...
## 18. 13차 개선 — 반복되는 실수 패턴을 4단계 총평에 녹이기

배경: 이전에 브레인스토밍했던 "보너스, 더 큰 작업" 아이디어(rootCauses 기반 반복 패턴 짚어주기)를 이번에 진행했습니다.

### 구현 ([App.tsx](src/App.tsx), [gemini.ts](src/services/gemini.ts))
- **[App.tsx](src/App.tsx) `solveStep`**: 이 학생의 과거 오답들(현재 건 제외, 이미 로드된 `mistakes` 목록 사용 — 추가 DB 조회 없음)에서 체크된 `rootCauses`를 전부 집계해서, **2회 이상 반복된 것 중 가장 많이 체크된 원인**을 `recurringRootCause`(`{ label, count }`)로 계산해 `solveMistakeWithGemini`에 새 파라미터로 전달합니다. 라벨은 `ROOT_CAUSE_OPTIONS`에서 찾아 이모지 접두사를 뗀 순수 텍스트로 넘깁니다.
- **[gemini.ts](src/services/gemini.ts) `solveMistakeWithGemini`**: `recurringRootCause` 파라미터를 추가하고, 있을 경우 "이 학생은 과거 오답들에서 '{라벨}'을 실수 원인으로 {count}번 체크한 이력이 있다"는 지침을 프롬프트에 주입합니다.
- **배치 위치**: 단원별 통계(인사말용)와 달리, 이 패턴은 **"### 4단계: 돌아보기 & 쌤의 한끝 팁"** 섹션에서 짚어주도록 지침을 별도로 배치했습니다 — 인사말에 너무 많은 개인화 멘트가 몰리는 걸 피하고, "반성/총평" 성격상 4단계가 더 자연스러운 자리라고 판단했습니다.
- 다그치는 어조를 피하고 따뜻하게 격려하는 톤을 유지하도록 명시했습니다 (기존 `sameChapterMistakeCount` 지침과 동일한 원칙).
