...
export const CHRONOLOGICAL_CHANGELOGS = [
  {
    ...
    date: '2026.07.24',
    changes: [
      'AI 진단 속도/안정성 대폭 개선 프로젝트 완료 — 무료 API 키를 폐지하고 유료 키 전용으로 전환하여 레이트리밋으로 인한 지연·오류 원천 차단',
      '문제 분류(과목/단원 판정) 단계 AI 사고 과정을 최적화하여 1차 분석 응답 속도 향상',
      '아무도 사용하지 않던 3단계 힌트 기능을 과감히 제거하고, 그만큼 해설 생성 속도를 함께 단축',
      '문제 풀이 해설을 실시간 스트리밍 방식으로 전면 개편하여, 전체 생성이 끝나기 전부터 완성되는 순서대로 화면에 바로바로 노출',
      '문제 지문/인쇄 영역 추출 작업을 과목 분류 작업과 완전히 동시에 처리하는 병렬 파이프라인 구조로 재설계',
      'Gemini 서버 일시 과부하(503) 발생 시 자동 재시도 로직을 탑재하여 진단 안정성 강화'
    ]
  },
  {
    version: 'v1.14.0',
    date: '2026.07.17',
    changes: [
      '하단 내비바 우측 상단 모퉁이에 "📝 복습하기" 플로팅 배지 버튼 전격 신설',
      '복습 완료되지 않은(3차 미완료) 오답들 중 가장 예전에 등록/활동된 오답 카드를 지능적으로 찾아 즉시 열어주는 자동 복습 세션 연동',
      '상세 모달창 우측 가장자리에 큼지막하고 선명한 "Next (다음)" 플로팅 화살표 버튼 탑재',
      'Next 화살표 터치 시 모달을 닫지 않고도 다음 미완료 오답 카드로 순차 롤링 전환되는 슬라이드 기능 완성',
      '카드가 넘어갈 때 이전 카드의 분석 데이터(오답 단원, 원인, 계획 등)가 묻어 나오지 않도록 폼 필드 상태 자동 동기화 리셋 탑재'
    ]
  },
  {
    version: 'v1.13.2',
    date: '2026.07.13',
    changes: [
      '하단 내비게이션 바 전체 레이아웃 높이를 아래로 5px 하강 패치 완료',
      '하단 내비바 좌측 상단 모퉁이에 실시간 동시 접속 학생 수를 세주는 "Online (숫자)" 플로팅 배지 신설',
      'Online 배지 터치 시 현재 함께 공부 중인 친구들의 실시간 이름 명단이 말풍선처럼 짜잔 하고 노출되는 팝업 UI 탑재',
      '서버 자원 절약 및 데이터 정확성을 보장하기 위해 30초 단위 초경량 Polling 연동'
    ]
  },
  {
    version: 'v1.13.1',
    date: '2026.07.13',
    changes: [
      '복습완료 탭 내 "실시간 공부자극! 친구들의 복습 현황" 위젯의 행을 어드민(교사용)이 터치 시 해당 오답의 상세 정보 모달이 열리도록 연동',
      '학생 오답 정보 보호를 위해 일반 학생 간 오답 상세 조회는 RLS 보안망으로 완벽 차단하고 오직 어드민만 클릭 권한 개방',
      '로컬에 캐싱되지 않은 오답 카드 조회 시 Supabase 단건 쿼리를 실시간 비동기 로딩하여 부드럽게 팝업 연동',
      '카드 로딩 순간을 직관적으로 알 수 있도록 펄스(Pulse) 로딩 애니메이션 피드백 탑재'
    ]
  },
  {
    version: 'v1.13.0',
    date: '2026.07.13',
    changes: [
      '단순히 과목만 보여주던 분석통계 탭을 상세 "단원별 아코디언 범주형 버블 차트(Bubble Chart)"로 대전환 완료',
      '데이터 부족 보완을 위한 기간 필터 선택기(전체 누적 / 최근 90일 / 최근 30일) 탑재 및 기본값을 전체 기간으로 설정',
      '어드민(교사용) 계정에서 학생 필터를 "전체 학생 합계(all)"로 설정 시 학급의 공통 약점을 뭉쳐서 보여주는 메가 버블 차트 지원',
      '오답 개수에 반응하여 방울 크기(12px ~ 28px)와 취약 등급 네온 색상(신호등 스펙트럼)이 동적 스케일링되는 고급 UI 연출 적용'
    ]
  },
  {
    version: 'v1.12.9',
    date: '2026.07.12',
    changes: [
      '어드민(교사용) 전용 복습완료 보관함 조회 권한 확장 (3차 복습은 끝마쳤으나 미완성 상태인 XOO, O★O 등도 추가 렌더링)',
      '복습 히스토리 및 시도 흔적 추적 보존을 위해 3차 복습 실패 시 리셋 자동 정돈 로직은 수동 모드로 원복 결정',
      '복습완료 보관함의 컴팩트 한 줄 목록 뷰에 각 학생의 3단계 도장판 진척 현황(oox, xo★ 등) 미니 진단 배지 연동'
    ]
  },
  {
    version: 'v1.12.8',
