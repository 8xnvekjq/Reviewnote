...
/**
 * Escape untrusted text before it is inserted through innerHTML.
 * KaTeX output and the small set of markup generated by this component are
 * restored/created only after the source text has been escaped.
 */
...
const parseMarkdownWithMath = (text: string, isPrintMode = false): string => {
  ...
  s = s.replace(/\$\$([\s\S]*?)\$\$/g, (_match, inner) => {
    ...
    try {
      ...
      const cleanFallback = escapeHtml(formatMathFallback(mathText));
  ...
  // 3. Escape all remaining source text before generating any trusted markup.
  // Math blocks are placeholders at this point and are restored at the end.
  s = escapeHtml(s);

  // 4. Translate bold formatting (**text**)
  s = s.replace(/\*\*(.*?)\*\*/g, '<strong class="font-bold text-white">$1</strong>');

  // 5. Translate Markdown lines (lists and headers)
  const lines = s.split('\n');
