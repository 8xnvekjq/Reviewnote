...
// 시험대비 분석 대상 학년 — 현재 운영 중인 학년만(고1, 고2). profilesGradeMap은
// get_profile_directory가 관리자 계정을 이미 제외하고 내려주므로 별도의 admin/테스트 계정
// 필터링이 필요 없다.
const SUPPORTED_GRADES = ['고1', '고2'];
export function ExamPrepStudentPicker({ mistakes, profilesMap, profilesGradeMap, onSelect }: Props) {
  const students = useMemo(() => {
    return Object.keys(profilesGradeMap)
      .filter(id => SUPPORTED_GRADES.includes(profilesGradeMap[id]))
      .map(id => {
      ...
      .sort((a, b) => a.grade === b.grade ? a.name.localeCompare(b.name, 'ko') : a.grade.localeCompare(b.grade, 'ko'));
            ...
            <button type="button" key={s.id} className="rn-examprep-student-card" onClick={() => onSelect(s.id)}>
              <span>
                ...
                <span className="rn-examprep-student-sub">
                  {s.grade} · {s.count > 0 ? `오답노트 ${s.count}건${s.latest ? ` · 최근 ${s.latest.slice(5, 10).replace('-', '/')}` : ''}` : '등록된 오답노트 없음'}
                </span>
              </span>
