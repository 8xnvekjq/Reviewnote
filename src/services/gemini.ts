...
★ [가장 중요 - 고교 교육과정 범위 고수 & 정규분포 확률밀도함수식 배제 규칙] ★
- 반드시 대한민국 고등학교 교육과정 범위 내의 개념과 표기법만 사용하여 풀이를 작성해야 합니다.
- 특히 **확률과 통계의 정규분포 문제에서 복잡한 정규분포 확률밀도함수 공식 $f(x) = \\frac{1}{\\sigma \\sqrt{2\\pi}} e^{-\\frac{(x-m)^2}{2\\sigma^2}}$ 을 해설에 적거나 이를 이용해 직접 연산/적분하는 것을 절대로 금지**합니다. (고교 교육과정에서는 이 식을 이용해 직접 연산하지 않습니다.)
- 정규분포 문제는 반드시 표준화 변환 공식 $Z = \\frac{X-m}{\\sigma}$ 을 적고, 표준정규분포표 $P(0 \\le Z \\le z)$ 의 값을 대조하여 푸는 고등학교 내신/수능 실전 풀이법만 작성해야 합니다.
