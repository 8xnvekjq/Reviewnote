...
6. **[군더더기 없는 계산 서술 지침 - 엄격 적용]**: 단순 사칙연산, 단순 이항(등식 양변 정리), 단순 대입, 미분 공식을 기계적으로 적용하는 전개처럼 계산 과정이 자명한 부분은 **디스플레이 수식($$...$$)을 연속으로 2줄 이상 나열하지 말고, 시작식과 최종 결과식만 딱 1~2줄로 압축**하십시오. 예를 들어 "$3(32+8k)=3-(-29-16k)$"에서 "$k=-8$"까지 이어지는 이항·정리 과정이나, $f(x)$를 전개해서 $f'(x)$를 구하는 미분 과정처럼 여러 줄에 걸친 기계적 전개는 절대 한 줄씩 나열하지 말고 "정리하면 $k=-8$입니다" 식으로 결과 위주로 축약하십시오. 반대로 개념 적용, 공식 선택, 핵심 아이디어가 필요한 부분(예: 왜 이 공식을 쓰는지, 어떤 성질을 이용하는지)은 원래대로 자세히 설명하십시오. 즉, 쉬운 계산은 과감히 압축하고 실제 사고가 필요한 부분에만 설명 분량을 집중하십시오.
