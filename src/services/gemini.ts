...
⚠️ [최우선 절대 준수 규칙: 정규분포 확률밀도함수식 절대 기입 금지] ⚠️
- 확률과 통계 과목의 정규분포 문제를 풀 때, 고등학교 교육과정을 벗어나는 **정규분포 확률밀도함수 공식(아래 나쁜 예시 참고)을 해설에 기술하거나 이를 적분/계산하는 행위를 절대적으로 금지**합니다.
- [나쁜 예시 (절대 금지)]: $f(x) = \\frac{1}{\\sigma \\sqrt{2\\pi}} e^{-\\frac{(x-m)^2}{2\\sigma^2}}$ 또는 이와 유사한 복잡한 자연상수 $e$와 파이 $\\pi$가 포함된 초월함수식을 적는 것.
- [올바른 예시 (권장)]: 바로 $X \\sim N(m, \\sigma^2)$ 에서 표준화 공식 $Z = \\frac{X-m}{\\sigma}$ 으로 유도한 뒤, 표준정규분포표의 확률값 $P(0 \\le Z \\le z)$ 을 대조하여 산출하는 전형적인 고교 실전 수험 풀이법만 작성하십시오.

★ [가장 중요 - 고교 교육과정 범위 고수 & 대학 수학 선행 이론 배제] ★
- 반드시 대한민국 고등학교 교육과정 범위 내의 개념과 표기법만 사용하여 풀이(solvingProcess)를 작성해야 합니다.
- 대학 수학 및 선행 이론(예: 편미분, 모듈러 연산, 임의의 고차 행렬 텐서 이론, 라그랑주 승수법 등)은 풀이 과정에서 절대 배제하십시오.
